...
def run_backtest(args):    
    ...
    # start_date="2020"
    print(rise, down, freq, take_profit, start_date)
    X = df["Close"].resample(f'{freq}T').last().loc[start_date:]
    df_pnl, pivots, actions, stats = backtest_long(X, rise, down, take_profit, False)
...
def main():
    ...
    args = [(a, c, b, d) for a in safe_arange(0.01, 0.12, 0.01) for c in safe_arange(-0.11, 0, 0.01) for b in [5, 15, 60, 720] for d in [10] + safe_arange(1, 2, 0.1).tolist()]
    # args = [(a, c, b, d) for a in safe_arange(0.01, 0.12, 0.01) for c in safe_arange(-0.11, 0, 0.01) for b in [1440] for d in [0]+safe_arange(0.6, 1, 0.05).tolist()]
    ...
    i = 1
    while True:
        file_name = f"BackTestLearn/data/zigzag_rank{start_date}_updown_profit_long v{i}.csv"
